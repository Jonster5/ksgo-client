...
export class GameMap implements GameMapObject {
        ...
        switch (m.version) {
            case 0.1:
                this.stars = m.stars.map(({ diameter, color, mass, x, y }) => {
                    diameter = typeof diameter === 'string' ? this.generate(diameter) : diameter;
                    mass = typeof mass === 'string' ? this.generate(mass) : mass;
                ...
                this.planets = m.planets.map(({ diameter, color, mass, x, y, vx, vy }) => {
                    diameter = typeof diameter === 'string' ? this.generate(diameter) : diameter;
                    mass = typeof mass === 'string' ? this.generate(mass) : mass;
                    x = typeof x === 'string' ? this.generate(x) : x;
                    y = typeof y === 'string' ? this.generate(y) : y;
                    ...
                    return new Planet(stage, { diameter, color, mass, x, y, vx, vy });
                });

                this.asteroids = m.asteroids.map(({ diameter, color, mass, x, y, vx, vy }) => {
                    diameter = typeof diameter === 'string' ? this.generate(diameter) : diameter;
                    mass = typeof mass === 'string' ? this.generate(mass) : mass;
                    x = typeof x === 'string' ? this.generate(x) : x;
                    y = typeof y === 'string' ? this.generate(y) : y;
                    ...
                    return new Asteroid(stage, { diameter, color, mass, x, y, vx, vy });
                });

                this.spawns = m.spawn.map(({ x, y, size }) => new Spawn(x, y, size));
                break;
            default:
    ...
    getSpawnCoords(): { x: number; y: number; r: number } {
        const { x, y } = this.spawns[this.rand(0, this.spawns.length)].getCoords();
