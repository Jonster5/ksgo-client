...
import type { GameOptions, MapItem, ShipStatObject } from '@data/types';
import { Writable, writable } from 'svelte/store';
import { Canvas } from './api/canvas';
import { Game, GameUtils, HostedGame } from './utils/gameUtils';
import { GameMap } from './map';
import { PlayerSpectator, PlayerShip, EnemyShip } from './ship';
import { Stage } from './api/stage';
import type { EnemyShipObject, PlayerShipObject, RemoteShipObject } from './ship';
import { GET_KSGO_ID, RemoteSendInfo } from '@data/multiplayer';
    ...
    init(m: MapItem) {
        ...
        this.canvas.update = () => {
            ...
            this.enemies.forEach((e) =>
                e.updateGravity(this.map.stars, this.map.planets, this.map.asteroids)
            );
            ...
            this.enemies.forEach((e) =>
                e.AI([this.user, ...Array.from(this.enemies)], { size: this.stage.width })
            );
